...
class SelfTransactionController {
  ...
  async create(req) {
    try {
      ...
      const normalizedTransactionType = String(transaction_type)
        .toLowerCase()
        .trim();
      console.log("Normalized transaction_type:", normalizedTransactionType);

      if (
        normalizedTransactionType !== "receive" &&
        normalizedTransactionType !== "pay"
      ) {
  ...
  async update(req) {
    try {
      ...
      const existingTransaction =
        await SelfTransactionRepository.readById(transaction_id);
      ...
      const transactionType =
        req_object.transaction_type || existingTransaction.transaction_type;
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const updatedTransaction = await SelfTransactionRepository.update(
          ...
          {
            ...req_object,
            account_id: req_object.account_id
              ? Number(req_object.account_id)
              : undefined,
            amount: req_object.amount ? Number(req_object.amount) : undefined,
            is_bank:
              req_object.is_bank !== undefined ? req_object.is_bank : undefined,
          },
          tx,
        );
