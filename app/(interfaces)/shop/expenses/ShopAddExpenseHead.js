...
export default function ShopAddExpenseHead() {
    const {
        register,
        handleSubmit,
        control,
    ...
    const handleAddSubHead = async () => {
        ...
        try {
            ...
            if (result.response_status === "success") {
                ...
                if (newSubHead && newSubHead.shop_subhead_id) {
                    setValue("shop_subhead_id", newSubHead.shop_subhead_id.toString());
                }
                setNewSubHeadName("");
    ...
    const onSubmit = async (data) => {
        const payload = {
            req_object: {
                ...
                shop_subhead_id: parseInt(data.shop_subhead_id),
                ...(isEditMode && { shop_account_id: editingAccountId }),
            },
        };

        try {
            const url = "/api/shopExpense/accounts";
            ...
            if (result.response_status === "success") {
                ...
                reset({ shop_subhead_id: data.shop_subhead_id, account_nam: "" });
                setIsEditMode(false);
    ...
    const handleEdit = (account) => {
        setIsEditMode(true);
        setEditingAccountId(account.shop_account_id);
        reset({
            shop_subhead_id: account.shop_subhead_id?.toString() || "",
            account_nam: account.account_nam || "",
        });
        document.getElementById("shop-account-form")?.scrollIntoView({ behavior: "smooth" });
    };
        ...
        <div className="p-3 sm:p-4 md:p-6 space-y-4">
            <Card className="max-w-2xl p-0! mx-auto">
                <CardContent className="p-4 sm:p-6">
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" id="shop-account-form">
                        <div className="space-y-2">
                                        ...
                                        <div className="flex-1">
                                            <Combobox
                                                options={subHeads.map((sh) => ({
                                                    value: sh.shop_subhead_id.toString(),
                            ...
                            {errors.shop_subhead_id && (
                                <p className="text-sm text-destructive">{errors.shop_subhead_id.message}</p>
                            )}
                        </div>
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-3">
                                {filteredAccounts.map((account) => (
                                    <Card key={account.shop_account_id} className="border shadow-sm">
                                ...
                                <table className="w-full caption-bottom text-sm">
                                    ...
                                    <tbody>
                                        {filteredAccounts.map((account, idx) => (
                                            <tr key={account.shop_account_id} className="hover:bg-muted/50 border-b">
