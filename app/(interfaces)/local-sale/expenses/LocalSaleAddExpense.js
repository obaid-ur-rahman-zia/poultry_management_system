...
export default function LocalSaleAddExpense() {
    const {
        register,
        handleSubmit,
        control,
        ...
        defaultValues: {
            ls_expense_date: new Date().toISOString().split("T")[0],
            ls_subhead_id: "",
            ls_account_id: "",
            amount: "",
            description: "",
    ...
    const selectedSubId = watch("ls_subhead_id");

    const accountsForSubHead = selectedSubId
        ? accounts.filter((account) => account.ls_subhead_id?.toString() === selectedSubId)
        : [];
    ...
    const fetchExpenseTransactions = async (date) => {
        setLoading(true);
        try {
            const url = date ? `/api/localSaleExpense/readAll?all=true&date=${date}` : `/api/localSaleExpense/readAll?all=true`;
    ...
    const fetchAllTransactionsForModal = async () => {
        ...
        try {
            const response = await fetch(`/api/localSaleExpense/readAll?all=true`);
            const result = await response.json();
            if (result.response_status === "success") {
                const data = result.response_result?.data || result.response_result || [];
    ...
    const onSubmit = async (data) => {
        const payload = {
            req_object: {
                ls_expense_date: data.ls_expense_date,
                ls_account_id: parseInt(data.ls_account_id),
                amount: parseFloat(data.amount),
                description: data.description?.trim() || "",
                ...(isEditMode && { ls_expense_id: editingTransactionId }),
            },
        };

        try {
            const url = "/api/localSaleExpense";
            ...
            if (result.response_status === "success") {
                ...
                reset({
                    ls_expense_date: data.ls_expense_date,
                    ls_subhead_id: data.ls_subhead_id,
                    ls_account_id: "",
                    amount: "",
                    description: "",
                });
    ...
    const handleEdit = (transaction) => {
        setIsEditMode(true);
        setEditingTransactionId(transaction.ls_expense_id);
        reset({
            ls_expense_date: transaction.ls_expense_date
                ? new Date(transaction.ls_expense_date).toISOString().split("T")[0]
                : new Date().toISOString().split("T")[0],
            ls_subhead_id: transaction.account?.ls_subhead_id?.toString() || "",
            ls_account_id: transaction.ls_account_id?.toString() || "",
    ...
    const handleConfirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const response = await fetch(`/api/localSaleExpense?ls_expense_id=${deletingTransactionId}`, { method: "DELETE" });
            const result = await response.json();
            if (result.response_status === "success") {
    ...
    const modalFilteredTransactions = allTransactions.filter((t) => {
        ...
        const matchesDate = !modalFilterDate || (t.ls_expense_date && new Date(t.ls_expense_date).toISOString().split("T")[0] === modalFilterDate);
        ...
        <div className="p-3 sm:p-4 md:p-6 space-y-4">
            <Card className="max-w-xl mx-auto">
                <CardContent className="p-4 sm:p-6">
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" id="ls-expense-form">
                        <div className="space-y-2 w-full">
                            <Label htmlFor="ls_expense_date">Date *</Label>
                            <div className="flex items-center gap-2">
                                <Input
                                    id="ls_expense_date"
                                    type="date"
                                    className="w-full"
                                    {...register("ls_expense_date", { required: "Date is required" })}
                                />
                            ...
                            {errors.ls_expense_date && <p className="text-sm text-destructive">{errors.ls_expense_date.message}</p>}
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Combobox
                                        options={subHeads.map((sh) => ({ value: sh.ls_subhead_id.toString(), label: sh.subhead_nam }))}
                                        value={field.value}
                                        onValueChange={(val) => {
                                            field.onChange(val);
                                            setValue("ls_account_id", "");
                                        }}
                            ...
                            />
                            {errors.ls_subhead_id && <p className="text-sm text-destructive">{errors.ls_subhead_id.message}</p>}
                        </div>

                        <div className="space-y-2 w-full">
                            <Label htmlFor="ls_account_id">Account *</Label>
                            <Controller
                                name="ls_account_id"
                                control={control}
                                ...
                                render={({ field }) => (
                                    <Combobox
                                        options={accountsForSubHead.map((acc) => ({ value: acc.ls_account_id.toString(), label: acc.account_nam }))}
                                        value={field.value}
                            ...
                            />
                            {errors.ls_account_id && <p className="text-sm text-destructive">{errors.ls_account_id.message}</p>}
                        </div>
            ...
            <Card>
                <CardContent>
                    <MobileListToggle title="Local Sale Expenses">
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-3">
                                {filteredTransactions.map((t, idx) => (
                                    <Card key={t.ls_expense_id} className="border shadow-sm">
                                        <CardContent className="p-3 space-y-2">
                                            <div className="flex justify-between"><span className="text-xs text-gray-500">Date</span><span className="text-sm">{new Date(t.ls_expense_date).toLocaleDateString()}</span></div>
                            ...
                            <div className="relative max-h-[600px] overflow-auto">
                                <table className="w-full caption-bottom text-sm">
                                    ...
                                    <tbody>
                                        {filteredTransactions.map((t, idx) => (
                                            <tr key={t.ls_expense_id} className="hover:bg-muted/50 border-b">
                                                <td className="p-2">{idx + 1}</td>
                                                <td className="p-2 whitespace-nowrap">{new Date(t.ls_expense_date).toLocaleDateString()}</td>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {modalFilteredTransactions.map((t) => (
                                            <tr key={t.ls_expense_id} className="border-b hover:bg-muted">
                                                <td className="p-2">{new Date(t.ls_expense_date).toLocaleDateString()}</td>
                                                <td className="p-2">{t.account?.account_nam}</td>
