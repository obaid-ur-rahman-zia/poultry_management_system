...
export default function PurchaseReportModal() {
  ...
  async function fetchSupplier() {
    const data = await fetch("/api/supplier/readAll");
    const resData = await data.json();
    const { response_result } = resData;
    const supplierResult = response_result.supplier_data;
  ...
  const fetchReport = async () => {
    if (!selectedSupplier && !selectedProduct) {
      toast.error("Please select Supplier or Product");
      return;
    }
    ...
    try {
      ...
      if (selectedSupplier && !selectedProduct) {
        res = await fetch(
          `/api/unitExpense/read/readReportDetail?start_dat=${startDate}&end_dat=${endDate}&supplier_id=${selectedSupplier}`
        );
      } else if (!selectedSupplier && selectedProduct) {
        res = await fetch(
          `/api/unitExpense/read/readReportDetail?start_dat=${startDate}&end_dat=${endDate}&product_id=${selectedProduct}`
        );
      } else {
        res = await fetch(
          `/api/unitExpense/read/readReportDetail?start_dat=${startDate}&end_dat=${endDate}&supplier_id=${selectedSupplier}&product_id=${selectedProduct}`
        );
      }
