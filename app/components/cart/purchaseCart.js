...
const Cart = ({ cartItems, onRemove, onUpdate, isPurchaseReturn }) => {
  ...
  const calculateAmounts = (item, updatedField, newValue) => {
    ...
    // Parse all necessary values
    const quantity = Number(updatedItem.quantity) || 0;
    const packing = Number(updatedItem.packing) || 1;
    const unitPrice = Number(updatedItem.unit_price) || 0;
    const bonus = Number(updatedItem.bonus) || 0;
    ...
    // Base amount = Purchase Price × Quantity
    const baseAmount = unitPrice * quantity;
    updatedItem.prod_subtotal_amount = baseAmount.toFixed(2);
    ...
    if (isEditing(item.product_id, field)) {
      return (
        <input
          ...
          step={field === "purchase_price" ? "0.01" : "1"}
          min="0"
        />
      );
    }
        ...
        <table className="w-full border-collapse mt-2">
          <thead>
            <tr className="bg-[#F9FAFC] text-[#858D95] shadow-sm sticky top-0">
              ...
              {!isPurchaseReturn && <th className="p-2">Bonus</th>}
              {isPurchaseReturn && <th className="p-2">Return Type</th>}
          ...
          <tbody>
            {filteredCart.map((item, index) => (
              ...
              >
                ...
                {isPurchaseReturn && (
                  <td className="p-2 text-center">
                    <span className="block p-1">{item.return_type_nam}</span>
                  </td>
                )}
                <td className="p-2 text-center">
                  <span className="block p-1">{item.product_nam}</span>
                </td>
                <td className="p-2 text-center">
                ...
                <td className="p-2 text-center">
                  <span className="block p-1">{item.expiry}</span>
                </td>
                <td className="p-2 text-center">
                  <span className="block p-1">{item.batch}</span>
                </td>
                <td className="p-2 text-center">
                  <button
            ...
            {filteredCart.length === 0 && (
              <tr>
                <td colSpan="11" className="text-center p-2 text-gray-500">
                  Cart is empty
                </td>
              </tr>
            )}
